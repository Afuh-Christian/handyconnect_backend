...
impl<T, IdType> BaseRepoTrait<T, IdType>
    ...
    where
        T: Send + Sync + for<'r> FromRow<'r, sqlx::postgres::PgRow> + Unpin + HasId<IdType> + ColumnsAndPlaceholdersTrait   ,
...
async fn get_all(&self) -> Result<Vec<T>, ApiError> {
    ...
    let data = sqlx::query_as::<_, T>(format!("SELECT * FROM {}", self.table_name).as_str())
...
async fn delete(&self, id: IdType) -> Result<OperationResult, ApiError> {
    sqlx::query(format!("DELETE FROM {} WHERE {} = $1", self.table_name, self.id_name).as_str())
        .bind(&id)
        .execute(&self.db_pool)
        .await
        .map_err(|_| ApiError {
...
async fn save_list(&self, items: Vec<T>) -> Result<OperationResult, ApiError>
{
    ...
    let query_str = format!(
        ...
        self.table_name,
        columns.join(", "),
        items.iter().map(|_| placeholders.join(", ")).collect::<Vec<_>>().join(", "),
        self.id_name,
        update_assignments.join(", ")
    );
